...
public class MenuMB implements Serializable {
	...
	public void cadastrarMenu() throws ProjetoException {
		...
		if (listaSistemasDual.getTarget().size() > 0) {
			...
			if (menu.getTipo().equals("menuItemRel")) {
				 ...
				 * else { menu.setDescPagina(null); menu.setDiretorio(null);
				 * menu.setExtensao(null); menu.setAction(null);
			...
			if (cadastrou == true) {
				...
				JSFUtil.fecharDialog("dlgACadMenu");
			} else {
	...
	public List<Menu> filtrarPreMenu(String tipo, List<Menu> listaCad,
			List<Menu> listaAlt) throws ProjetoException {
	...
	public MenuModel gerarMenusPreview(List<Menu> listaMenusPreview,
			Integer idSistema) {
	...
	private void recActionOnclick() {
		...
		if (menu.getTipo().equals("menuItemRel")) {
			if (menu.getAction() != null && menu.getOnclick() == null) {
				tipoMenuRel = "A";
				menu.setAction(menu.getAction().replace("#{", "")
						.replace("}", ""));
			}

			if (menu.getAction() == null && menu.getOnclick() != null) {
				tipoMenuRel = "D";
				menu.setOnclick(menu.getOnclick().replace(".show();", ""));
			}
		}
	}
