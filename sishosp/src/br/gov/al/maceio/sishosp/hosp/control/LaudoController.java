...
public class LaudoController implements Serializable {
    ...
    public boolean idadeValida() {
    	...
    	if(!valido)
    		JSFUtil.adicionarMensagemErro("A idade do paciente n�o compreende o intervalo permitido entre idade minima e m�xima", "");
    	return valido;
    }
    ...
    public boolean validaCidsDoLaudo() {
    	...
    	for (CidBean cidBean : listaCidsLaudo) {
			if(!lDao.validaCodigoCidEmLaudo(cidBean.getCid(), this.laudo.getDataSolicitacao(), this.laudo.getProcedimentoPrimario().getCodProc())) {
				JSFUtil.adicionarMensagemErro("Cid(s) selecionado(s) incompat�veis com o permitido no SIGTAP ", "");
				return false;
			}
		}
