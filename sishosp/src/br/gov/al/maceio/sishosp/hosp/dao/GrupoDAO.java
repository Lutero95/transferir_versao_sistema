...
public class GrupoDAO {
	...
	public boolean gravarGrupo(GrupoBean grupo) throws SQLException {
		...
		String sql = "insert into hosp.grupo (descgrupo, qtdfrequencia, auditivo, tipo_exame_auditivo) values (?, ?, ?, ?);";
		try {
			...
			ps.setString(4, grupo.getTipoExameAuditivo().toUpperCase());
			ps.execute();
			con.commit();
			return true;
	...
	public List<GrupoBean> listarGruposPorPrograma(int codPrograma) {
		...
		String sql = "select g.id_grupo, g.descgrupo, g.qtdfrequencia, g.auditivo, g.tipo_exame_auditivo from hosp.grupo g, hosp.grupo_programa gp, hosp.programa p"
		...
		try {
			...
			while (rs.next()) {
				...
				grupo.setTipoExameAuditivo(rs.getString("tipo_exame_auditivo"));
	...
	public List<GrupoBean> listarGrupos() {
		List<GrupoBean> lista = new ArrayList<>();
		String sql = "select id_grupo, descgrupo, qtdfrequencia, auditivo, tipo_exame_auditivo from hosp.grupo order by id_grupo";
		try {
			System.out.println("CHEGOU AQUI");
			con = ConnectionFactory.getConnection();
			PreparedStatement stm = con.prepareStatement(sql);
	...
	public List<GrupoBean> listarGruposBusca(String descricao, Integer tipo) {
		List<GrupoBean> lista = new ArrayList<>();
		String sql = "select id_grupo, descgrupo, qtdfrequencia, auditivo, tipo_exame_auditivo from hosp.grupo ";
		if (tipo == 1) {
	...
	public Boolean alterarGrupo(GrupoBean grupo) throws ProjetoException {
		String sql = "update hosp.grupo set descgrupo = ?, qtdfrequencia = ?, auditivo = ?, tipo_exame_auditivo = ? where id_grupo = ?";
		try {
			con = ConnectionFactory.getConnection();
			...
			stmt.setString(4, grupo.getTipoExameAuditivo().toUpperCase());
	...
	public GrupoBean listarGrupoPorId(int id) {
		...
		GrupoBean grupo = new GrupoBean();
		String sql = "select id_grupo, descgrupo, qtdfrequencia, auditivo, tipo_exame_auditivo from hosp.grupo where id_grupo = ?";
		try {
			con = ConnectionFactory.getConnection();
			PreparedStatement stm = con.prepareStatement(sql);
			stm.setInt(1, id);
